    ...
    if (kpage != NULL)
    {
        ...
        if (success){
            *esp = PHYS_BASE;
            ...
            int argc = 0;
            int bytec = 0;
            char **args;  //array to keep arguments
            char **argv;
            char *token, *save_ptr; //to keep a token and a save_ptr by using character pointers
            //make a copy of file name
            fn_copy = palloc_get_page (0);
            if (fn_copy == NULL)
                return TID_ERROR;
            strlcpy (fn_copy, file_name, PGSIZE);
//split aargumanets into tokens and save to array
            args = malloc(DEFAULT_ARG_SIZE*sizeof(char *));
            ...
            //save arguments strings to stack
            int str_len;
            argv= malloc(argc * sizeof(char*) );
            for (int i = (argc-1); i >= 0; i--)
            {
                str_len = strlen(args[i]+1);
                bytec += str_len;
                *esp -= str_len;
                argv[i]=*esp;  //keep track of pointers to strings

                memcpy(*esp, args[i],str_len);
            }
            ...
            //null pointer to inidacte end of argv
            *esp -= sizeof(char *);
            *(char *) *esp = 0;

            //now input into the stack from the argv array that is reverse order
            //save argv values into stack
            ...
            *esp -= sizeof(int);
            memcpy(*esp,&argc,1);
            // save return address (fake one for now)
            *esp -=sizeof(void*);
            hex_dump((int)* esp, *esp, bytec*4, 1);
            free(args);
            free(argv);
            palloc_free_page(fn_copy);
