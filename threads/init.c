...
char * read(void);
int compare(char *arg,char *arg1);
...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    // TODO: no command line passed to kernel. Run interactively
      ...
      char *whomai=malloc(sizeof(char)*7);
      whomai="whomai";
      ...
      while(1)
      {
          ...
          if (compare(word,whomai)){
             ...
             printf("%s\n","threads");
              ...
              printf("%s","Exiting the Qemu emulator ..bye!");
              break;
          }
...
int compare(char* word1,char* word2){
    ...
    while(1)
    {
        if (word1[count]==word2[count] && word1[count]==0x00){return 1;}
        else if(word1[count]==word2[count]){count++;}
        else{return 0;}
    }
}
char * read(void){
